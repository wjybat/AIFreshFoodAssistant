...
使用 OpenCode Zen/订阅提供的 GLM-5.2：

```dotenv
LLM_PROVIDER=opencode
LLM_BASE_URL=<OpenCode 订阅提供的 Base URL>
LLM_MODEL=glm-5.2
```

OpenCode 将 GLM-5.2 作为 OpenAI-compatible Chat Completions 模型提供，但不接受智谱专属 `enable_thinking`/`thinking` 扩展；`opencode` 配置不会发送这些字段，从而避免 `Extra inputs are not permitted`。模型是否内部进行推理以及是否返回 `reasoning_content` 由 OpenCode 网关决定。
...
| LLM_REASONING_EFFORT | GLM-5.2 思考强度 | max |
