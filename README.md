...
`IMAGE_API_KEY` 留空时复用 `LLM_API_KEY`。默认 `IMAGE_GENERATION_REQUIRED=false`，因此图片服务不可用时仍会保存方案并回退到原有 Emoji 与 HTML 菜谱；如果部署要求所有菜品必须有图片，可将其设为 `true`。当前示例端点为 HTTP，只适合受控演示环境，生产部署应使用可信 HTTPS 服务。

### 4. 启用本地隔离的开发 MCP 数据服务（可选）

门店经营数据库和 MCP 服务端放在项目根目录的 `.tmp\AIFreshFoodAssistant-mcp-dev`。服务源码、种子、SQLite 数据和依赖锁会随仓库共享；本地 `.venv`、缓存及运行日志不会提交：
...
该数据源固定标记为 `store-operational-data`，供当前门店经营分析使用。仓库随附的 SQLite 是种子 fixture，不能替代已授权的生产 ERP 数据；接入外部 ERP/数据库时，只需保持相同三个只读工具契约，远程服务应使用 HTTPS 和鉴权。
...
`.tmp/AIFreshFoodAssistant-mcp-dev` 中的门店经营数据和 MCP 服务。镜像以非 root
...
本仓库的 `.tmp/AIFreshFoodAssistant-mcp-dev` 提供独立的只读门店经营数据服务，所有
返回均标记为 `store-operational-data`。它通过专用
...
| 文件 | 场景 | 天气 | 关键库存 | 社区画像 |
|------|------|------|----------|----------|
| test_store_s1_rainy.json | 雨天晚餐 | 小雨 18°C | 青椒(1天临期)/猪肉(高库存)/豆腐(2天) + 42 SKU | 家庭客群 |
| test_store_s2_hot.json | 高温清凉 | 36°C | 黄瓜(1天临期)/冬瓜(高库存)/排骨(2天) + 42 SKU | 白领+居民 |
| test_store_s3_weekend.json | 周末家庭 | 晴 25°C | 五花肉(高库存)/番茄(2天临期) + 42 SKU | 家庭聚餐 |
| test_store_s4_winter.json | 冬至节日 | 冬至 2°C | 猪肉馅(2天临期)/白菜(1天临期)/面粉(高) + 42 SKU | 北方社区 |

JSON 至少需要 `store_info`、`weather`、`community` 和非空 `inventory`。其中 `store_info.date` 是方案保存和自动恢复的必填业务键，建议使用稳定格式，如 `2026-07-15`。

每份 JSON 都包含稳定的门店 ID、业务日期及 50 个 SKU。库存卡每页显示 10 条，使用翻页按钮查看其余商品。
