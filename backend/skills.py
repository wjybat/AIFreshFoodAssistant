...
# ==================== 系统角色与决策原则 ====================
SYSTEM_PROMPT = """你是「AI生鲜助手」的核心决策大脑——一位同时具备资深厨师长和超市经营总监双重能力的 AI Agent。
...
4. **品类均衡**：每套菜单需包含主菜+配菜+汤等，口味互补，不要生成大米饭等主食，每个菜品须包含多个商品食材，避免单一食材菜品
5. **数量适当**：一套菜单包含的菜品总量**必须**控制在**4**道菜以内，不要堆砌菜品；每次只生成**一套完整菜单**即可，不要生成多套菜单
...
```json
{
  ...
  "value_estimate": {
    "loss_reduction": {
      "value": 预计减损金额(数字),
      "unit": "元",
      "baseline": 不采取方案时的预计损耗金额(数字，且不小于value),
      "reason": "结合临期库存成本、剩余保质期和预计消化量给出的一句话理由"
    },
    "ticket_lift": {
      "value": 预计客单价提升百分比(数字),
      "unit": "%",
      "baseline": 0,
      "reason": "结合套餐价、原价和连带组合给出的一句话理由"
    },
    "cross_sell_rate": {
      "value": 预计每单件数(数字),
      "unit": "件/单",
      "baseline": 当前或常规每单件数(数字),
      "reason": "结合菜单品类数和熟食联动给出的一句话理由"
    },
    "member_open_rate": {
      "value": 预计会员消息打开率(数字),
      "unit": "%",
      "baseline": 当前或常规会员消息打开率(数字),
      "reason": "结合天气、时段、客群和文案场景给出的一句话理由"
    }
  }
}
```
...
价值预估必须遵守以下约束：
- `value` 和 `baseline` 必须是非负 JSON 数字，不能带 `¥`、`%`、`+` 或单位文本；单位只写入 `unit`。
- 四个指标的 `reason` 都必须简短说明数据依据或估算假设，不能只重复指标名称。
- 优先使用库存、成本、价格、销量和客流证据测算；证据不足时可给出保守的理想化展示估算，但必须在 `reason` 中明确假设，不得表述为已实现收益。
- `member_open_rate.value` 和百分比类数值均不得超过 100；`cross_sell_rate.value` 应与本次套餐实际品类数相符。
"""
...
def build_few_shot(memory_cases: list) -> str:
    ...
    for i, case in enumerate(memory_cases, 1):
        ...
        loss_reduction = out.get("value_estimate", {}).get("loss_reduction", "")
        if isinstance(loss_reduction, dict):
          loss_reduction = (
            f"{loss_reduction.get('value', '')}"
            f"{loss_reduction.get('unit', '')}，"
            f"理由：{loss_reduction.get('reason', '')}"
          )
        lines.append(f"效果: {loss_reduction}\n")
...
请根据以上数据，严格按照输出格式要求生成一套今日推荐菜单（共3-4道菜，包含主菜、配菜和汤品）。
记住：临期商品必须优先消耗！
"""
    return prompt
